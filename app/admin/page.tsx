        ...
        for (const line of lines) {
            if (line.startsWith('title:')) {
                ...
                while (idx < lines.length && lines[idx].startsWith(' - ')) {
                    tagLines.push(lines[idx].replace(' - ', '').trim());
                    idx++;
                }
                data.tags = tagLines.join(',');
            }
        }
    ...
    if (!isAuth) {
        return (
            <div className="min-h-screen bg-gray-950 text-gray-100 flex items-center justify-center p-8">
                <form onSubmit={handleLogin} className="bg-gray-900 border border-gray-700 rounded p-8 max-w-sm w-full">
        ...
        <div className="min-h-screen bg-gray-950 text-gray-100 flex flex-col">
            ...
            <div className="border-b border-gray-700 bg-gray-900 px-6 py-3 flex justify-between items-center">
                <div className="flex items-center gap-3 flex-1">
                    <span className="text-gray-400 text-sm">shane-blog / posts /</span>
                    <input
                        type="text"
                        value={fileName}
